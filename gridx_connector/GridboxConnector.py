...
class GridboxConnector:
    ...
    def get_gateway_id(self, max_retries=3, retry_delay=60):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                last_error = err
                self.logger.error(f"Failed to retrieve gateway IDs (attempt {attempt}/{max_retries}): {err}")
    ...
    def retrieve_live_data_by_id(self, id):
        try:
            ...
            self.logger.error(f"Failed to retrieve live data for system {id}: {err}")
            return None
    ...
    def retrieve_historical_data_by_id(self, id, start, end, resolution="15m"):
        ...
        try:
            ...
            self.logger.error(f"Failed to retrieve historical data for system {id}: {err}")
            return None
